...
!real*8 :: start_time, stop_time, current_time1,current_time2
...
call cpu_time(start_time) ;print*,"start",start_time
!call cpu_time(current_time1) ;print*,"start",current_time1
!print*,"start",start_time,"curr",current_time1
total_gabriel=0;print*,total_gabriel
call getarg(2,cu)
...
call getarg(1,carg)
precarg=carg
!open(18,file=trim(carg)//"movement")
!open(28,file=trim(carg)//"GF")
!!open(38,file=trim(carg)//"WA")
!open(48,file=trim(carg)//"induction")
!open(58,file=trim(carg)//"phys")
!open(68,file=trim(carg)//"terri")
!open(78,file=trim(carg)//"complexity")
!open(88,file=trim(carg)//"geneTotal")
!open(98,file=trim(carg)//"geneVar")
!open(108,file=trim(carg)//"terriN")
!open(118,file=trim(carg)//"aconection")
...
winame="w"!precaa//cazero(3:10)//cae//caf//cag!//char(0)
...
select case(aut)
case(1)
 ! print*,"Changing reqmin, screen radius, tor, stor...forcing real time -->ffu(21)=1"
  reqmin=0.1 !w1
  screen_radius=0.85 !w!
 ! node(:)%stor=90d0; node(:)%tor=10d0 !w!
  getot=0 ; rtime=0
!  ffu(21)=1
 ! gen(1)%wa(1)=1d0 
...
case default
  ...
  print*,"Changing reqmin, screen radius..."
  reqmin=0.1 !w1
  screen_radius=0.85 !w!
