...
real*8,  public                ::reqsmax          !maximal size of epi cell
...
real*8, public, allocatable    ::vcilx(:),vcily(:),vcilz(:),divisa(:)!,ccrate(:)   !force components storage arrays (differential equations method) !>>>>> Miquel 20-6-13
...
real*8,public,allocatable :: nodeu(:,:),nodev(:,:)
integer,public :: ndu, tellme=0
integer,public :: endtimer
integer,public :: tellme2=0, once=0
integer,public :: ndv=0
integer, public                ::nd00
...
integer, public, allocatable   :: gex00(:,:),gex01(:,:)
real, public, allocatable   :: gexAll0(:,:),gexAll1(:,:)
real*8, public :: rdiffmax  !>>Miquel24-2-14
real*8, public :: nodeoini(35,2)


  


real*8,public                 ::start_time, stop_time, current_time1,current_time2,total_gabriel
...
!     real*8                    ::reqmax         ! THIS IS A MODEL PARAMETER is the maximal req in a cell before one can add a new node
     ...
     real*8                     ::lineage  ! IS 7-10-15

end type

type(nod), public, allocatable :: node(:),ulte(:),nodeo(:),nodeoo(:),node00(:),node01(:) ! Is 14-3-15 notice nodeo saves the initial conditions and for the new nodes Is 25-5-13
...
!!!! THIS IS JUST FOR THE NOISE PART::::::NEW VARIABLES DEFINED
! stress per node: fmeanl(node)
! energy per node: node(node)%e
real*8, public :: contact_inhibition=0.0  ! Strength of contact inhibition
real*8, public :: growth_polarization=0.5 ! Proportion of influence of polarization gradient versus physical gradient
real*8, public :: growth_asynchronous=0.5 ! 0.0 cell cycle synchronized; 1.0 cell cycle random
real*8, public :: slowed_down=1.0 ! this is multiplied to the minimal displacement; 0.1 makes movements 10 times slower
real*8, public :: temp_modifier=1.0 ! this is multiplied to the temperature
real*8, public :: growth_modifier=1.0 ! this is multiplied to growth rate 
real*8, public :: ini_cellphase=0.5 ! this is the initial cell phase (if synchronous) 0.5
!!!! THESE VARIABLES MODIFY IN MODEL and SINGLE_NODE SUBROUTINES
real*8, public :: finishes=0.0 ! MODIFY THIS TO END DEVELOPMENT AT THIS rtime
integer, public :: ffu25=0 ! ffu25 is gradual divisions
integer, public :: lowerthreshold=1 ! lower gex threshold ON against numerical artefacts
integer, public :: fixedborders=0 ! put a hold on the boarders
integer, public :: chequerboard=0 ! prevents neighbour cells from dividing after one another
integer, public :: distclose=0 ! 1: new nodes are placed on top of their sisters
integer, public :: ndoo,ndi,ndur,yet=0
integer, public :: done1=0
integer, public :: all_dgex_start=0
real*8, public :: rtimes=0d0
integer, public, allocatable :: border(:,:)
integer, public, allocatable :: ccrate(:)
!!!! THIS MODIFIES NEXUS. VERY IMPORTANT!!!!!!!
