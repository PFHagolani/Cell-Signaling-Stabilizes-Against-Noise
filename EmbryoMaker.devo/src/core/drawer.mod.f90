...
subroutine display
  integer irf
  character*100 org
  character*140 :: pernofidraw
   ! pernofidraw="output/99nghomo/99nghomo.dat"
 ! pernofidraw="output/99ng99nddiffu/99ng99nddiffu.dat"
  !   pernofidraw="output/99ng99ndhighdiffu/99ng99ndhighdiffu.dat"
   !   pernofidraw="output/99ng99nd10/99ng99nd10.dat"
 !  pernofidraw="optimum.dat"
  !  pernofidraw="optimum_helmet.dat12119.dat"
 !  pernofidraw="output/99ng99ndinverse/99ng99ndinverse.dat"
  !   pernofidraw="output/99ng99ndrandom/99ng99ndrandom.dat"
 ! pernofidraw="output/mod_mexican/optimum.dat9993.dat"
  ! pernofidraw="output/99_hetero_genes/99_hetero_genes.dat"
 ! pernofidraw="output/manygeneshetero/manygeneshetero.dat"
!  pernofidraw="2894598999_161449elli.e___Feb2__16__0.dat________6171____________40_________10.dat10521.dat_________40max"
 !  pernofidraw="output/smallflat/548179664__152610elli.e___Feb16_15_10.dat507.dat"
   ! pernofidraw="optimum_helmet.dat12119.dat"
 !  pernofidraw="optimum.dat9993.dat"
  !  pernofidraw="99nghomo.dat"
 ! pernofidraw="99_hetero_genes.dat"
  !  pernofidraw="optimum_generation5.dat"
 !  pernofidraw="optimum_smallmaod.dat"
 !  call fit(1,a,pernofidraw)
...
real*8  oldx,oldy,oldz,pfh_aa,pfh_bb,pfh_cc
...
!        call glvertex3d(aa*esca,bb*esca,cc*esca)
          color(1)=0 ; color(2)=1
         call cylinder3(i,aa,bb,cc,3,color)
      end if
    end do
  end if
  ...
  if (flag(23)==1.or.flag(27)==1) then			!>>>>>>>>>>>>>>>>>>>>>>>>>> Miquel 18-6-13
    ...
    do i=1,nd
      if(node(i)%tipus==1)cycle ! PFH OJO
      ...
      if (flag(27)==1) then 
        pfh_aa=a+px(i)*aaa ; pfh_bb=b+py(i)*aaa ; pfh_cc=c+pz(i)*aaa
      else
        bbb=1d0/sqrt(px(i)**2+py(i)**2+pz(i)**2)
        pfh_aa=a+px(i)*bbb ; pfh_bb=b+py(i)*bbb ; pfh_cc=c+pz(i)*bbb
      end if
      if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
     !   call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,red)
     !   call glvertex3d(a*esca,b*esca,c*esca)
     !   call glvertex3d(aa*esca,bb*esca,cc*esca)
         ...
         call cylinder3(i,pfh_aa,pfh_cc,c,3,color)
      end if
    end do
    ...
    do i=1,nd
     ...
     !   call glvertex3d(aa*esca,bb*esca,cc*esca)
         ...
         call cylinder3(i,pfh_aa,pfh_bb,pfh_cc,3,color)
      end if
    end do
...
!    case(40) ; !do i=1,nd 
    ...
    case(41) ; mine=minval(fmeanl(1:nd)) ; maxe=maxval(fmeanl(1:nd)) !!!!!!!ESTA LINEA!!!!!! 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if(custom_arrowselection==arrowselection)then
    ...
    case(40) ; do i=1,nd 
     ...
     case(41) ; amine=minval(fmeanl(1:nd)) ; amaxe=maxval(fmeanl(1:nd)) !!!ESTA LINEA!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if(custom_sphereselection==sphereselection)then
   ...
   ! case(41) ; smine=1 ; smaxe=nd
    case(41) ; smine=minval(fmeanl(1:nd)) ; smaxe=maxval(fmeanl(1:nd)) !!!!!ESTA LINEA!!!!!!!!!!!!!
...
!    maxe=maxe*100
   ! mine=mine/100
    mine=maxe/1000   ! PFH cambiando max color
...
!print*,"arrowselection",arrowselection
  ...
  if(flag(12)==1.or.flag(13)==1)then   
    do i=1,nd
      ...
      if(node(i)%tipus>3.and.flag(32)==0) cycle
      if(flag(36)==0.and.node(i)%hold==1) cycle  !>>>>Miquel15-1-14
      if(node(i)%tipus==2) cycle ! PFH OJO
      if((flag(30)==1.and.node(i)%tipus<3).or.(flag(31)==1.and.node(i)%tipus==3).or.(flag(32)==1.and.node(i)%tipus==4))then
        a=node(i)%x ; b=node(i)%y ; c=node(i)%z
        if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
          do j=1,nneigh(i)
            ic=neigh(i,j) ; if(node(ic)%tipus==2) cycle ! PFH OJO
              ...
              if((flag(30)==1.and.node(ic)%tipus<3).or.(flag(31)==1.and.node(ic)%tipus==3).or.&
                ...
                if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
                  ...
                  if(d<node(i)%da+node(ic)%da)then     !>>>>>> Is 9-5-13
                    if (flag(12)==1) then
                      if(node(i)%tipus==1)then;color=0;color=0!(1)=1
                     ! else;color=1;color(3)=0;end if
                       else;color=1;end if	
                    else
                  ...
                  !   call cylinder2(i,ic,3,color)   ! what draws the cylinder, number, resolution
                  end if
                end if
              end if
            end if
...
real(GLFLOAT)::color(4),calor(4)
  ...
  r=0.035d0 ; riv=0.025d0  !r: radio del cilindro
...
subroutine cylinder3(nod,vx,vy,vz,res,color)  !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Miquel 15-5-13
integer::nod,iv,res,i,j,k,ii,jj,kk
real*8::r,riv,cx,cy,cz,ix,iy,iz,vx,vy,vz,d
real*8::rad,ax,ay,az,ux,uy,uz,thet
real*8::bx,by,bz,ox,oy,oz,cost,sint,ucost
real(GLFLOAT)::color(4),calor(4)
  ...
  r=0.03!0.035d0 
  riv=0.025d0  !r: radio del cilindro
  ix=cels(nod)%cex ; iy=cels(nod)%cey ; iz=cels(nod)%cez
  !ix=node(nod)%x ; iy=node(nod)%y ; iz=node(nod)%z
 ! vx=node(iv)%x ; vy=node(iv)%y ; vz=node(iv)%z
  cx=vx-ix ; cy=vy-iy ; cz=vz-iz
  d=sqrt(cx**2+cy**2+cz**2)
...
case(15)
!  call fit(1,a,"shared_volumen_long_1/352.dat________352max")
...
!    end if
  ...
  case (5)
	...
	call draw_func
  case (6)
      !confirm cursor selection
      if(flag(7)==0)then ; print*,"CHOOSE A SELECTION MODE FIRST"; return ; end if
      if(flag(4)==1)then
        call selectcell(cursx,cursy,cursz)
        if (cellid>0) then
          nki=nki+1
...
!!!! COLOR MENUS  !>>Miquel30-10-14
  ...
  call glutAddMenuEntry("as mechanical stress"//char(0),41)    !!!!!ESTA LINEA!!!!!!!
...
call glutAddMenuEntry("selection mode: select node by index"//char(0),1)!>>Miquel24-10-14
call glutAddMenuEntry("selection mode: cell by index"//char(0),3)!>>Miquel24-10-14
call glutAddMenuEntry("selection mode: select node with cursor"//char(0),2)!>>Miquel24-10-14
call glutAddMenuEntry("selection mode: select cell with cursor"//char(0),4)!>>Miquel24-10-14
call glutAddMenuEntry("confirm selection with cursor"//char(0),6)!>>Miquel24-10-14
