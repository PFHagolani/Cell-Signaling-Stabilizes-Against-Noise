...
subroutine nexe
  ...
  real*8 :: scui,maxa,maxs(3),mins(3),u_e,minda!,u_d
...
!    call cellbreak ! to see if the cell is split in two 
 ...
 if(nd>5000)then; print*, "HAS BECOME TOO BIG"; 
  whichend=5
  goto 171
 endif 
...
!if((cputime-start_time)>72000)then
!  whichend="Over 10 hours 2"
!  goto 171 
!end if


! RZ: WE NEED TO MAKE SURE THE MORPHOLOGY ACTUALLY FINISHES
...
if(tellme2==0)then ! store at the first iteration. This could also be done in a different way. !tellme2 en general

  all_dmov=1d0; all_dmov_max=0d0
  tellme2=getot
  if(allocated(nodeu)) deallocate(nodeu)
  allocate(nodeu(nd,3))
  do i=1,nd
    nodeu(i,1)=nodeo(i)%x;nodeu(i,2)=nodeo(i)%y;nodeu(i,3)=nodeo(i)%z
  end do
  ndu=nd

end if

if((mod(int(rtime/deltamax),10)==0).and.(getot>tellme2))then  ! check every 10 iterations ARBITRARY
  ...
  do l=1,ndu
    ...
    difk=difk+node(l)%diffe
    if(node(l)%diffe<vvv) vvv=node(l)%diffe
    if(node(l)%diffe>uuu) uuu=node(l)%diffe
    scui=sqrt((node(l)%x-nodeo(l)%x)**2+(node(l)%y-nodeo(l)%y)**2+(node(l)%z-nodeo(l)%z)**2)+scui ! total displacement
    if(node(l)%x<mins(1)) mins(1)=node(l)%x  ! store extreme points
    if(node(l)%x>maxs(1)) maxs(1)=node(l)%x
    if(node(l)%y<mins(2)) mins(2)=node(l)%y
    if(node(l)%y>maxs(2)) maxs(2)=node(l)%y
    if(node(l)%z<mins(3)) mins(3)=node(l)%z
    if(node(l)%z>maxs(3)) maxs(3)=node(l)%z

    !if((rtime>10*10).and.(getot>1000*10))then  ! check after some while how fast morphogenesis has proceded
    if(getot>10)then
      all_dmov=all_dmov+sqrt((node(l)%x-nodeu(l,1))**2+(node(l)%y-nodeu(l,2))**2+(node(l)%z-nodeu(l,3))**2)
      if(all_dmov>all_dmov_max) all_dmov_max=all_dmov ! displacement per 10*deltamax rtime, ca. 10 getot
      if(alld<sqrt((node(l)%x-nodeu(l,1))**2+(node(l)%y-nodeu(l,2))**2+(node(l)%z-nodeu(l,3))**2))then
        alld=sqrt((node(l)%x-nodeu(l,1))**2+(node(l)%y-nodeu(l,2))**2+(node(l)%z-nodeu(l,3))**2)
      endif
    else
      all_dmov=0.0001 ! we never end up there
    endif
  ...
  allocate(nodeu(nd,3))
  do i=1,nd
    nodeu(i,1)=node(i)%x;nodeu(i,2)=node(i)%y;nodeu(i,3)=node(i)%z  ! update positions saved
  end do
  ndu=nd

  maxa=maxs(1)-mins(1)
  if(maxs(2)-mins(2)>maxa) maxa=maxs(2)-mins(2)
  if(maxs(3)-mins(3)>maxa) maxa=maxs(3)-mins(3)
  scui=scui/(maxa*nd)  ! total displacement, not in use
  ...
  ! if a node gets 3 times the original size of node 1 we kill the program
  if (mod(getot,1000).eq.0) then !>>> Is 4-2-14
...
mx1=1000d0;my1=1000d0;mz1=1000d0;mz3=1000d0;mx2=-1000d0;my2=-1000d0;mz2=-1000d0;mz4=-1000d0;ndm=0;k=1;l=1
do i=1,nd
  if(node(i)%tipus>2) cycle
  if((node(i)%z.lt.mz3).and.(node(i)%tipus<3))then; mz3=node(i)%z; k=i; endif
  if((node(i)%z.gt.mz4).and.(node(i)%tipus<3))then; mz4=node(i)%z; l=i; endif
  if(node(i)%x.lt.mx1) mx1=node(i)%x
  if(node(i)%y.lt.my1) my1=node(i)%y
  if(node(i)%z.lt.mz1) mz1=node(i)%z
  if(node(i)%x.gt.mx2) mx2=node(i)%x
  if(node(i)%y.gt.my2) my2=node(i)%y
  if(node(i)%z.gt.mz2) mz2=node(i)%z
  if(node(i)%tipus==3) ndm=ndm+1
end do

! CHECK IF THERE IS ANY GROWTH INTO z
 !Check cpu_time
call cpu_time(cputime)

if((done1==0).and.(cputime>1500).and.(nd>1.5*ndoo).and.(rtime>0.5))then
  a=0.5*(abs(node(k)%z-node(node(k)%altre)%z)+abs(node(l)%z-node(node(l)%altre)%z))
  if(abs(mz3-mz4).lt.1.1*a.and.nd>1.5*ndoo)then ! a bit funky
    whichend=1
    goto 171 ! OUT: too flat, no morphogenesis
  else
    done1=1
  endif
endif
...
! check black holes and broken morphologies
if(mod(getot,500)==0)then
  c=0; cc=0
  do i=1,nd
    if(node(i)%tipus.ne.1) cycle
    b=0
    l=0; aa=1000000d0
    do j=1,nneigh(i)
      k=neigh(i,j)
      if(k==i) cycle
      a=sqrt((node(i)%x-node(k)%x)**2+(node(i)%y-node(k)%y)**2+(node(i)%z-node(k)%z)**2)
      if(a<0.05*node(i)%req) b=b+1
      if((a<aa).and.(node(i)%altre.ne.k).and.(node(k)%tipus<3))then; aa=a; l=k; endif
    end do 
    if(l==0) cycle
    if(node(l)%tipus.ne.node(i)%tipus) cc=1+cc ! broken epithelium
    if(b>1) c=c+1 ! black hole
  end do
  if(c>0.01*ndoo)then
    whichend=12
    goto 171
  endif
  if(cc>0.05*ndoo)then
    whichend=13
    goto 171
  endif
endif
...
! Check cpu_time
!call cpu_time(cputime)
!print*, cputime, "cputime", cputime-cputime0
!if(cputime>1200)then ! OUT: TOO FAST DIVISIONS
!  whichend=2
!  goto 171 ! OUT: too fast divisions; ca.4%
!end if

! Check total expansion of morphology: a diameter of 50LE is arbitrary, but filters out explosions
if((abs(mx1-mx2).gt.50d0).or.(abs(my1-my2).gt.50d0).or.(abs(mz1-mz2).gt.50d0))then
  whichend=3
  goto 171 ! OUT: too large
end if

if(first==0)then; first=1; rtime_a=0; rtime_c=0; endif
...
if(nd>5000)then; print*, "HAS BECOME TOO BIG"; 
  whichend=5
  goto 171
endif  ! OUT:SAVE too many nodes, may be regarded failure or right: ca.15%

! Check movement at a late time
if(rtime>100)then ! New: check if movement has gone down
  if(all_dmov<0.2*all_dmov_max)then
    whichend=6
    goto 171  ! THIS ONE IS FUNKY, BUT RARELY APPLIES
  endif
end if
if(getot>50000)then ! TIME OVER!!! Make sure automaticon.mod doesnt stop it before that
  whichend=7
 goto 171  ! WE NEVER GET THERE ACTUALLY
end if
if(rtime>500)then ! TIME OVER!!! Make sure automaticon.mod doesnt stop it before that
  whichend=8
 goto 171  ! OUT:SAVE NEVER FINISHED, should be regarded failure: ca.2%
end if

! Check cell division rate: only one division/rtime per cell
!nope

!>>> Is 4-4-14
if(rtime-rtime_c.ge.1)then; rtime_c=rtime_c+1; nd_p=nd; endif


!Check if the morphology still moves
!#! return

if((rtime>5).and.(rtime<6).and.(all_dmov<nd*delta))then ! ca.500 iterations before discarding ! OUT: NO MORE MOVEMENTS
  whichend=11
  print*, "NO MORE MOVEMENT", all_dmov; goto 171 ! most morphologies stop here
endif !UNDO
...
171  print *,""
  print *," THIS IS THE END all cells are differentiated and then the simulations stop",trim(carg)//trim(nofi)
  print *,""
