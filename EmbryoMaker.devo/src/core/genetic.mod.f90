...
real*8,  public, allocatable :: dgex(:,:),cgex(:,:) ! the increment: only for calculations
...
!  character*100        ::label      !this is intended to be used as a label to identify and track the gene (trhough evolution for example) !>>Miquel8-8-14
  ...
  real*8, allocatable  ::wa(:)    ! effect of a gene on each node parameter. These are ordered by number; after the number of the node parameter
                                  ...
                                  ! nparam_per_node+14= ECM proteolysis (like apoptosis for ECM nodes)
...
subroutine initiate_gene  ! allocates the matrices and fills npag and whonpag
  ...
  if (allocated(gem)) deallocate(gem,gep)
  allocate(gem(ng),gep(ng))
...
!if(ng==11)then;gen(11)%kindof=1; gen(11)%mu=0d0; gen(11)%diffu=0d0; gen(11)%npost=0; gen(11)%npre=0;endif
  ...
  do i=1,ng
    if (gen(i)%kindof<1) gen(i)%kindof=1  !every gene should have a kindof, if not we give the 1 value, for transcripts
!print *,i,"i",ng, gen(i)%kindof,int(gen(i)%kindof)
...
!do i=1,nd
!do j=1,ng
    !if(gex(i,j)+delta*dgex(i,j)>0.000001) agex(:nd,1:ng) = gex(:nd,1:ng) + delta*dgex(:nd,1:ng)   
...
!ldi=1d-8!epsilod
  do i=1,nd
    do j=1,ng
      if (abs(agex(i,j))<ldi.and.lowerthreshold==1) agex(i,j)=0d0
     !if ((gex(i,j))<ldi) gex(i,j)=0
    end do
  end do
...
!      rdiffmax=2*maxval(node(:nd)%da)*dmax !now it's in the iteracio subroutine !>>Miquel27-2-14
      do i=1,nd
        ...
        ! DIFFUSION
!if(i.ne.298.or.getot<1882)then
        do i1=1,nneigh(i)
          ie=neigh(i,i1)
          dist=dneigh(i,i1)
...
!! RZ: NO PRIMARY FORMS : AS WE USE ONLY THE w-MATRIX, EVERY FORM IS "PRIMARY"
    ...
    !!!      end do 
do kkj=2,6 ! RZ  (2) -> (kkj)
          do jjj=1,nkindof(kkj)           !*****************PRODUCTION OF PRIMARY FORMS (WITH FURTHER FORMS)
            j=wkindof(kkj,jjj)
            a=gex(i,j)
            sm=0.0d0
            do k=1,int(nw(j))
...
!OJO to maintain a gradient on the x axis of the last gen ... **** PFH
...
!      gex(i,ng)=x
!enddo
!maxpfh=maxval(gex(:,ng))
!gex(:,ng)=gex(:,ng)/maxpfh
...
!if(getot==1) call system("rm gg ")
!if(mod(getot,10)==0)then
!open(12,file="gg",position="append")
!do i=1,ng
!  gem(i)=maxval(gex(:,i))
!  gep(i)=minval(gex(:,i))
!end do
...
!  j=1
!  if(mod(i,2)==0) j=2
!  !write(12,*) getot, i, node(i)%x, node(i)%y, node(i)%z, gex(i,:), j, gem(:), gep(:)
!end do
close(12)
!endif
...
!!open(12,file="gexx1",position="append")
!!open(13,file="gexx2",position="append")
!!do i=1,nd
!!  if(goood(i)==1) write(12,*) getot, i, node(i)%x, gex(i,:)
!!  if(goood(i)==2) write(13,*) getot, i, node(i)%x, gex(i,:)
!!end do
!!close(12)
!!close(13)
